...
{ /* current --> proposal (=current->next) --> current ; final acceptance from calling function */
  ...
  for (i = 0; i < tre->n_undone; i++) { /* scan all nodes with d_done = false, but updating only children */ 
    if (tre->undone[i]->left->d_done) phy->l[ tre->undone[i]->left->id  ]->d_proposal = phy->l[ tre->undone[i]->left->id  ]->d_current;
    else                              phy->l[ tre->undone[i]->left->id  ]->d_proposal = phy->l[ tre->undone[i]->left->id  ]->d_current->next;

    if (tre->undone[i]->right->d_done) phy->l[ tre->undone[i]->right->id  ]->d_proposal = phy->l[ tre->undone[i]->right->id  ]->d_current;
    else                               phy->l[ tre->undone[i]->right->id  ]->d_proposal = phy->l[ tre->undone[i]->right->id  ]->d_current->next;
  }
...
{ /* accepted --> proposal (by array index, not linked list) --> accepted */
  ...
  for (i = 0; i < tre->n_undone; i++) { /* scan all nodes with d_done = false, but updating only children */ 
    if (tre->undone[i]->left->d_done) phy->l[ tre->undone[i]->left->id  ]->d_proposal = phy->l[ tre->undone[i]->left->id  ]->d[0];
    else                              phy->l[ tre->undone[i]->left->id  ]->d_proposal = phy->l[ tre->undone[i]->left->id  ]->d[idx];

    if (tre->undone[i]->right->d_done) phy->l[ tre->undone[i]->right->id  ]->d_proposal = phy->l[ tre->undone[i]->right->id  ]->d[0];
    else                               phy->l[ tre->undone[i]->right->id  ]->d_proposal = phy->l[ tre->undone[i]->right->id  ]->d[idx];
  }
